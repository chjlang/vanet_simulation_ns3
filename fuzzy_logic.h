...
FIS* initialize_fis(char *fisFile)
{
	FIS *fis;
	int debug = 0;
	DOUBLE **fisMatrix;
	...
	fisMatrix = returnFismatrix(fisFile, &fis_row_n, &fis_col_n);
...
double* fisGetParameters(FIS *fis, double* &learning_rate, int &size)
{
	/*get parameters of membership functions*/
	//get number of the overall mf parameters
	int nparas = 0;
	for(int i = 0; i < fis->in_n; i++)
		for(int j = 0; j < fis->input[i]->mf_n; j++)
			nparas += fisGetMfParaN(fis->input[i]->mf[j]->type);

	for(int i = 0; i < fis->out_n; i++)
		for(int j = 0; j < fis->output[i]->mf_n; j++)
			nparas += fisGetMfParaN(fis->output[i]->mf[j]->type);

	//mf_para = (DOUBLE*) fisCalloc(nparas, sizeof(DOUBLE));
	double *mf_para = new double[nparas];
	learning_rate = new double[nparas];
	size = nparas;
	...
	for(int i = 0; i < fis->in_n; i++)
		for(int j = 0; j < fis->input[i]->mf_n; j++)
			for(int k = 0; k < fisGetMfParaN(fis->input[i]->mf[j]->type); k++)
			{
				mf_para[start] = fis->input[i]->mf[j]->params[k];
				learning_rate[start++] = fabs(fis->input[i]->bound[0] - fis->input[i]->bound[1])/150;
			}

	for(int i = 0; i < fis->out_n; i++)
		for(int j = 0; j < fis->output[i]->mf_n; j++)
			for(int k = 0; k < fisGetMfParaN(fis->output[i]->mf[j]->type); k++)
			{
				mf_para[start] = fis->output[i]->mf[j]->params[k];
				learning_rate[start++] = fabs(fis->output[i]->bound[0] - fis->output[i]->bound[1])/150;
			}

	printf("size_mf_array: %d\n", size);
	return mf_para;
}

void fisUpdateParameters(FIS *fis, double *mf_paras)
{
	printf("begin updating mf paras\n");
	int start = 0;
	for(int i = 0; i < fis->in_n; i++)
		for(int j = 0; j < fis->input[i]->mf_n; j++)
			for(int k = 0; k < fisGetMfParaN(fis->input[i]->mf[j]->type); k++)
				fis->input[i]->mf[j]->params[k] = mf_paras[start++];

	for(int i = 0; i < fis->out_n; i++)
			for(int j = 0; j < fis->output[i]->mf_n; j++)
				for(int k = 0; k < fisGetMfParaN(fis->output[i]->mf[j]->type); k++)
					fis->output[i]->mf[j]->params[k] = mf_paras[start++];

	//call this function whenever mf parameters have been modified
	printf("after updating mf paras\n");
	fisComputeOutputMfValueArray(fis, 101);

}
/***********************************************************************
fuzzy inference for calculate urgency degree
 **********************************************************************/
double fuzzy_logic(FIS *fis, double argu1, double argu2) 
{
	DOUBLE* outputArray = (DOUBLE*) fisCalloc(fis->out_n, sizeof(DOUBLE));
	DOUBLE* inputArray = (DOUBLE*) fisCalloc(fis->in_n, sizeof(DOUBLE));
	inputArray[0] = argu1;
	inputArray[1] = argu2;

	getFisOutput(inputArray, fis, outputArray);

	double result = outputArray[0];
...
/***********************************************************************
 * fuzzy inference for grouping
 ***********************************************************************/
double fuzzy_logic_grouping(FIS *fis, double argu1, double argu2, double argu3, double *mf_paras, bool &flag_update)
{
	if(flag_update)
	{
		fisUpdateParameters(fis, mf_paras);
		flag_update = false;
	}
